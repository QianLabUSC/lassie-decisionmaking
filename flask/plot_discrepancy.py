...
x = np.arange(0, 22, 1) 
y3 = discrepancy_reward

best_locs = output.get('discrepancy_locs')
best_spatial = output.get('discrepancy_reward')

plt.scatter(best_locs, best_spatial, marker="s", s=500, label="best locations")
...
plt.plot(x, y3, color="black", label="discrepancy reward", linewidth=1.5)
...
plt.savefig(my_path + '\\figs_test\\Discrepancy.png')
