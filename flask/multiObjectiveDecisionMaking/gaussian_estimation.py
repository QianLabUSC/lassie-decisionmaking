...
if __name__ == "__main__":
    ...
    plt.plot(np.linspace(0, 1, 1000), y_pred, label='Prediction')
    plt.fill_between(np.linspace(0, 1, 1000).ravel(), y_pred - 1.96*y_std, y_pred + 1.96*y_std, alpha=0.5, color='k', label='Uncertainty')
    plt.legend()

    plt.figure()
    plt.plot(np.linspace(0, 1, 1000), information_gaussian1)
    plt.legend()
    plt.show()
