...
Returns:
    ...
    model:
'''
def hypofit2variable(xx, yy, predicted_input):
...
class DecisionMaking:
    ...
    def handle_spatial_information_gaussian(self):
        ...
        shear = self.current_state_shear_strength
        y_pred, I_s, y_std = Gaussian_Estimation(location, shear, np.linspace(0, 22, self.density), False, 1, 5, 10)
        return y_pred, I_s, y_std
    def handle_discrepancy_information_gaussian(self):
        pass
    ...
    def handle_discrepancy_coverage(self):
        ...
        self.std_shearstrength_each_loc = []
        for jj in range(22):
            ...
            if(self.mean_moisture_each[jj] <= xx_mean[0]):
                ...
                std_shear_strength = std_f_font(self.mean_moisture_each[jj])
            elif(self.mean_moisture_each[jj] >= xx_mean[len(xx_mean)-1]):
                mean_shear_strength = f_end(self.mean_moisture_each[jj])
                std_shear_strength = std_f_end(self.mean_moisture_each[jj])
            else:
                mean_shear_strength = f(self.mean_moisture_each[jj])
                std_shear_strength = f_std(self.mean_moisture_each[jj])
            
            self.mean_shearstrength_each_loc.append(mean_shear_strength)
            self.std_shearstrength_each_loc.append(std_shear_strength)
    ...
    def handle_discrepancy_gaussian(self):
        mositure_pred, information_mois, mois_std, mois_noise = Gaussian_Estimation(self.location_flattend,
                         self.moisture_flattend, np.linspace(1, 22, 220), 
                         True, 1, 5, 10)
        ...
        return self.discrepancy_gaussian, self.feature_gaussian, self.noise_estimation
