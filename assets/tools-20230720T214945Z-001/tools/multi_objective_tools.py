from tools.pareto import *
from tools.weigted_objective import * 
k_info_signal = 0.4
...
def run_multi_objective_odsf(vector, info_level, human_reported_type, info_signature, disp_signature, noise_level, k_info_signal=0.6, k_noise=1.5, k_info=1, k_disp=1):
    """
    This function uses multi-objective optimization to evaluate the trade-offs between information gain and 
    discrepancy based on the reward vector and user preferences. 

    Parameters:
    vector (np.ndarray): A n by 2 array representing rewards. vector[:, 0] is the information reward 
    and vector[:, 1] is the discrepancy reward.

    info_level (float): The current information coverage.

    human_reported_type (str): The decision-making type reported by the user. It can be "info focused", 
    "info hierarchy", "trade-off", "disp focused", or "disp hierarchy" corresponding to [0, 0.25, 0.5, 0.75, 1.0]
    ...
    Returns:
    The function should return the optimized results based on the given parameters, but the exact output 
    ...
    normalized_vector = normalize_columns(vector)
    pareto_sets, utopia_point, dist_to_utopia, pareto_sets_loc = pareto_optimal_sets(normalized_vector)
    ...
    indices = np.isin(pareto_sets_loc, np.array([info_focused_location, info_hier_location, disp_focused_suggestion, disp_hier_location]), invert=True)  # Get indices of elements not equal to values
