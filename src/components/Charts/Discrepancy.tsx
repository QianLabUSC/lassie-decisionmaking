...
const DiscrepancyChart: React.FC<Props> = ({ width, height }) => {
  ...
  useEffect(() => {
    ...
    const cumulativeDiscrepancies =
      calculateCumulativeDiscrepancies(discrepancies);

    const informationRewards = calculateInformationRewards(discrepancies);
    const cumulativeInformationRewards =
      calculateCumulativeDiscrepancies(informationRewards);
    ...
    const yScale = d3
      .scaleLinear()
      .domain([0, d3.max(cumulativeDiscrepancies) ?? 0])
      .range([chartHeight, 0]);

    const yInfoRewardScale = d3
      .scaleLinear()
    ...
    const legend = svg
      ...
      .attr(
        'transform',
        (d, i) =>
          `translate(${chartWidth - margin.right},${chartHeight - margin.bottom - i * 20})`
      );
